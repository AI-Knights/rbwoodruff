...
const Sidebar = ({
  ...
  const routes = [
    { name: "Overview", icon: BarChart3, path: "/employer-dashboard" },
    {
      name: "Post a Job",
      icon: Users,
      path: "/employer-dashboard/job-post",
    },
    { name: "Manage Jobs", icon: Shield, path: "/employer-dashboard/manage-jobs" },
    { name: "Hired Applicants", icon: UserCheck, path: "/employer-dashboard/hired-applicants" },
    { name: "Scheduled Interviews", icon: CalendarClock, path: "/employer-dashboard/scheduled-interviews" },
  ];

  // Helper function to check if route is active
  const isRouteActive = (routePath: string) => {
    // Exact match for employer-dashboard overview
    if (routePath === "/employer-dashboard") {
