...
import { useGetUserRosterQuery } from "@/store/api/agencySlice/agencySlice";

const ITEMS_PER_PAGE = 15;

export default function UsersTable() {
  const [search, setSearch] = useState("");
  const [complianceFilter, setComplianceFilter] = useState<"all" | "on_track" | "delayed" | "non_compliant" | "completed">("all");
  const [currentPage, setCurrentPage] = useState(1);

  const { data: users = [], isLoading, isError } = useGetUserRosterQuery();
              ...
              <TableBody>
                {paginatedUsers.map((user) => (
                  <TableRow key={user.id} className="hover:bg-gray-50">
                    ...
                    <TableCell>
                      <Link href={`/agency-dashboard/user-roster/${user.id}`}>
