...
export default function UserDetails({ id: propId }: UserDetailsProps) {
  ...
  const timeline = data.timeline.map(item => ({
    ...
    type: item.event_type.includes('court') ? 'court' : 'milestone'
  }));

  const resumeSections = [
    { title: "Contact Info", status: data.resume.contact_info ? "Completed" : "Incomplete" },
    { title: "Work Experience", status: data.resume.work_experience ? "Completed" : "Incomplete" },
    { title: "Skills", status: data.resume.skills ? "Completed" : "Incomplete" },
  ];
    ...
    <div className="w-full min-h-screen">
      <div className="max-w-[1920px] mx-auto space-y-6 grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-5">
        ...
        <Tabs defaultValue="overview" className="w-full col-span-1 md:col-span-2 xl:col-span-4">
          <TabsList className="grid w-full grid-cols-3 h-auto px-4 py-4 border-2 rounded-full bg-gray-100">
          ...
          <TabsContent value="overview" className="mt-6">
            <UserOverview
              ...
              resumeStatus={`${data.resume.resume_completeness}% Complete`}
