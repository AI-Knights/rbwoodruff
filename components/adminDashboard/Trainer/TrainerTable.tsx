...
import { trainers } from '@/data/Trainer.data';
...
const TrainerTable = () => {
  ...
  const [selectedEmployer, setSelectedEmployer] = useState<Trainer | null>(null);
  const [showProfileDialog, setShowProfileDialog] = useState(false);
  const [showSuspendDialog, setShowSuspendDialog] = useState(false);
  const [showApproveDialog, setShowApproveDialog] = useState(false);
  const itemsPerPage = 8;

  const filteredEmployers = trainers.filter(trainer => {
    const matchesSearch = trainer.companyName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         trainer.id.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesFilter = filterStatus === 'All Users' || trainer.status === filterStatus;
  ...
  const handleViewProfile = (trainer: Trainer) => {
    setSelectedEmployer(trainer);
    setShowProfileDialog(true);
  };

  const handleSuspendClick = (trainer: Trainer) => {
    setSelectedEmployer(trainer);
    setShowSuspendDialog(true);
  };

  const handleApproveClick = (trainer: Trainer) => {
    setSelectedEmployer(trainer);
