            ...
            <table className="w-full">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">Trainer Name</th>
                  ...
                  <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900">Total Programs</th>
              ...
              <tbody className="divide-y divide-gray-200">
                {uiTrainers.map((trainer) => (
                  <tr key={trainer.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{trainer.id.slice(0, 8)}...</td>
                    <td className="px-6 py-4 text-sm text-gray-900">{trainer.name}</td>
                    <td className="px-6 py-4 text-sm text-gray-900">{trainer.specialization}</td>
                    <td className="px-6 py-4 text-sm text-gray-900">{trainer.experience}</td>
                    <td className="px-6 py-4 text-sm text-gray-900">{trainer.total_programs}</td>
                    <td className="px-6 py-4">
                      <span
                        className={`inline-flex items-center px-3 py-1 rounded-md text-sm font-medium ${
                          trainer.status === 'verified' ? 'bg-green-100 text-green-800' :
            ...
            <Card key={trainer.id}>
              <CardHeader className="pb-3">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-sm text-gray-500">ID: {trainer.id.slice(0, 8)}...</p>
                    <h3 className="font-semibold text-lg">{trainer.name}</h3>
                  </div>
                  <span
                    className={`inline-flex items-center px-2 py-1 rounded-md text-xs font-medium ${
                      trainer.uiStatus === 'Verified' ? 'bg-green-100 text-green-800' :
            ...
            {selectedTrainer && (
              <div className="grid grid-cols-2 gap-6 pt-2">
                ...
                <div>
                  ...
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-md text-sm font-medium ${
                      selectedTrainer.status === 'verified' ? 'bg-green-100 text-green-800' :
